...
def next_day(black_tiles_curr: Set[Tuple[int, int, int]]):
    neighbour_with_black = [neighbours(black_tile) for black_tile in black_tiles_curr]

    interested_tiles = [neighbour for neighbour_list in neighbour_with_black for neighbour in neighbour_list]
    interested_tiles += [black_tile for black_tile in black_tiles_curr]

    unique_tiles = set(interested_tiles)
    ...
    for tile in unique_tiles:
        ...
        if is_black(black_tiles_curr, tile):
            if black_neighbours_len == 1 or black_neighbours_len == 2: #negation of the criteria
...
def main():
    ...
    cur_tile = black_tiles_set
    for i in range(0, 100):
        cur_tile = next_day(cur_tile)
        print('day '+str(i+1)+' : '+str(len(cur_tile)))
