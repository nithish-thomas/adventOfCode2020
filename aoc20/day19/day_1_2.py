...
def parse_rules(rules_inp):
    ...
    def evaluate_rule(rule_num) -> Set[str]:
        ...
        for and_rule in or_rule_arr:
            res = set()
            for rule_num_in_and_rule in and_rule:
                string_in_rule = evaluate_rule(int(rule_num_in_and_rule))
                if len(res) == 0:
                    res = string_in_rule
                else:
                    new_set = set()
...
def main():
    ...
    all_messages_fn = parse_rules(rules_inp)
    all_messages = all_messages_fn(0)
    print('42')
    print(all_messages_fn(42))
    print('31')
    print(all_messages_fn(31))
