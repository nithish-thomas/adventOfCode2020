...
def next_itr(inp_arr: List[List[str]]):
    ...
    def count_seat(inp_arr, i, j, step_i, step_j):
        if i < 0 or i >= rows:
            return 0
        if j < 0 or j >= cols:
            return 0
        ...
        if inp_arr[i][j] == '#':
            ...
            return 0
        return count_seat(inp_arr, i+step_i, j+step_j, step_i, step_j)

    def next_itr_at(inp_arr: List[List[str]], i: int, j: int):
        cur_state = inp_arr[i][j]
        filled_seat = count_seat(inp_arr, i - 1, j - 1, -1, -1) + count_seat(inp_arr, i - 1, j, -1, 0) + count_seat(inp_arr, i - 1, j + 1, -1, 1) \
                      + count_seat(inp_arr, i, j - 1, 0, -1) + count_seat(inp_arr, i, j + 1, 0, 1) \
                      + count_seat(inp_arr, i + 1, j - 1, 1, -1) + count_seat(inp_arr, i + 1, j, 1, 0) + count_seat(inp_arr, i + 1, j + 1, 1, 1)
        if cur_state == 'L':
            if filled_seat == 0:
                return '#'
        elif cur_state == '#':
