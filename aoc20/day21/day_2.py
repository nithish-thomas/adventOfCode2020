...
def main():
    ...
    list_of_allergen_to_ingredient = list(allergen_in_ingredients.items())
    test = [(allergen, list(ingredient)[0]) for allergen, ingredient in list_of_allergen_to_ingredient]
    test = sorted(test, key=lambda x: x[0])
    res = [ingredient for _, ingredient in test]
    print(res)
    print(','.join(res))
